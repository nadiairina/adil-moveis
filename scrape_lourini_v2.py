...
def search_lourini(query):
    ...
    try:
        ...
        for i in imgs:
            src = i.get('src')
            if src and 'uploads' in src and query.lower().split()[0] in src.lower():
                return src
...
for pid, p in products.items():
    ...
    print(f"Trying: '{query}' for '{p['name']}'")
    img_url = search_lourini(query)
    if img_url:
...
print(f"Total downloaded in v2: {found_count}")
